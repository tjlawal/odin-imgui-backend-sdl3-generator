...
Key :: enum c.int { // Forward declared enum type ImGuiKey
	...
	GamepadFaceLeft,    // X           | Y       | Square   | Toggle Menu. Hold for Windowing mode (Focus/Move/Resize windows)
	GamepadFaceRight,   // B           | A       | Circle   | Cancel / Close / Exit
	GamepadFaceUp,      // Y           | X       | Triangle | Open Context Menu
	GamepadFaceDown,    // A           | B       | Cross    | Activate / Open / Toggle. Hold for 0.60f to Activate in Text Input mode (e.g. wired to an on-screen keyboard).
...
TableFlags_Reorderable       :: TableFlags(1<<1) // Enable reordering columns in header row. (Need calling TableSetupColumn() + TableHeadersRow() to display headers, or using ImGuiTableFlags_ContextMenuInBody to access context-menu without headers).
...
TableFlags_ContextMenuInBody :: TableFlags(1<<5) // Right-click on columns body/contents will also display table context menu. By default it is available in TableHeadersRow().
// Decorations
...
MultiSelectFlags_SelectOnMask_ :: MultiSelectFlags{.SelectOnAuto,.SelectOnClickAlways,.SelectOnClickRelease}
...
Style :: struct {
	...
	SeparatorSize:                    f32,           // Thickness of border in Separator()
...
ListClipper :: struct {
	...
	UserIndex:        c.int,            // Helper storage for user convenience/code. Optional, and otherwise unused if you don't use it.
...
foreign lib {
	...
	@(link_name="ImGui_TreeNodeGetOpen")           TreeNodeGetOpen           :: proc(storage_id: ID) -> bool                                                             --- // retrieve tree node open/close state.
	// Widgets: Selectables
	...
	@(link_name="ImGuiViewport_GetDebugName")            Viewport_GetDebugName            :: proc(self: ^Viewport) -> cstring ---
